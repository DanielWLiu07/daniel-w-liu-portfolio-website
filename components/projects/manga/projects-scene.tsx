...
import { hatchMaterial, PAGE_KEY, type HatchCoords } from "./hatch-material";
...
function ProjectCard({
  ...
  edge: THREE.Material;
  subject: THREE.Material;
  ball: boolean;
}) {
  const geo = useMemo(() => new THREE.BoxGeometry(CARD.w, CARD.h, CARD.d), []);
  const faceGeo = useMemo(
    () => new THREE.BoxGeometry(CARD.w * 0.86, CARD.h * 0.86, CARD.d * 0.5),
    [],
  );
  ...
  const sphere = useMemo(
    () => new THREE.SphereGeometry(SUBJECT_SIZE * 0.42, 48, 32),
    [],
  );
...
export default function ProjectsScene({
  ...
  // The three knobs that decide whether the hatch READS, on the query string so
  // a comparison is one reload rather than one edit. `co` is the space the
  // strokes are drawn in, `key` is the irradiance the rig normalises to (which
  // is what moves the hatch band across the surface), `size` is the frequency.
  const num = (k: string, d: number) => {
    const v = Number(params.get(k));
    return Number.isFinite(v) && params.has(k) ? v : d;
  };
  const coords = (params.get("co") as HatchCoords | null) ?? undefined;
  const key = num("key", PAGE_KEY);
  // 3, not his group's 5. His 5 is in UV space across one square unwrap; in the
  // ship's own coordinates the same number lands about 40 strokes across the
  // hull, which at this camera distance is finer than a pixel and aliases into
  // grain rather than reading as strokes.
  const size = num("size", 3);
  // The CARD stays paper whichever shader is up. Hatching the panel as well as
  // the subject was the first thing tried and it eats the silhouette: figure and
  // ground come out at the same density and the ship stops being a shape. Manga
  // inks the subject and leaves the panel to paper or a screentone, and the
  // reason is exactly this. ?hatchall puts it back if it needs to be seen.
  const hatchAll = params.has("hatchall");
  const stock = useMemo(
    () =>
      useHatch && hatchAll
        ? hatchMaterial({ coords, key, hatchA: size, hatchB: size })
        : cardMaterial(),
    [useHatch, hatchAll, coords, key, size],
  );
  const edge = useMemo(
    () =>
      useHatch && hatchAll
        ? hatchMaterial({ coords, key, hatchA: size * 1.6, hatchB: size * 1.6 })
        : edgeMaterial(),
    [useHatch, hatchAll, coords, key, size],
  );
  ...
  return (
    <>
      {/* Paper, as the SCENE's background rather than the page's, and OUTSIDE
          the rig group because attach binds to the parent object: inside the
          group this sets a background on a Group, which nothing reads, and the
          sky stays black.
          The canvas is transparent, so with the print pass off the CSS behind it
          shows through and the sky reads as paper; with the pass on, the frame
          goes through a render target whose empty pixels are the clear colour,
          the pass reads that as black, and the sky printed near-solid ink above
          the horizon. The page colour has to be IN the scene for the pass to
          print it as paper. */}
