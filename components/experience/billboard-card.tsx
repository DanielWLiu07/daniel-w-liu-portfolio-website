...
export function BillboardCard({ experience }: BillboardCardProps) {
  ...
  return (
    <div className={`experience-card relative group w-full max-w-2xl md:max-w-[740px] overflow-visible mx-auto md:mx-0 [transform:rotateZ(0deg)] md:[transform:rotateZ(1deg)] ${isComingSoon ? 'opacity-60' : ''}`}>
      ...
      <div
        className="relative rounded-lg border-3 border-gray-300 p-3 md:p-4 shadow-2xl transition-all duration-500 ease-out group-hover:shadow-3xl min-h-[100px] md:min-h-[140px] z-10 group-hover:-translate-x-1 group-hover:-translate-y-1 opacity-80 group-hover:opacity-100 group-hover:brightness-110"
        style={{
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-baseline gap-2 md:gap-3 mb-1 md:mb-2">
              <h3 className={`text-base md:text-xl font-bold ${isComingSoon ? 'text-gray-500' : 'text-gray-900'}`}>
                {experience.title}
              </h3>
              <p className={`text-xs md:text-sm ${isComingSoon ? 'text-gray-400' : 'text-gray-600'}`}>
                {experience.period}
              </p>
            </div>
            <p className={`text-sm md:text-lg font-semibold mb-1 ${isComingSoon ? 'text-gray-500' : 'text-gray-700'}`}>
              {experience.company}
            </p>
            <p className={`text-[10px] md:text-xs leading-relaxed ${isComingSoon ? 'text-gray-400 italic' : 'text-gray-700'}`}>
