...
export function dealerCardReveal(age=Infinity) {
  if(!Number.isFinite(age)||age>=1.5) return {visible:true,prepare:0,release:1,grip:1,thumbGrip:1,wind:0,flick:0,point:1,bank:0,wrist:0,lift:0,reach:0,side:0,fan:1,turn:0,slide:.067,support:1,width:1}
  const prepare=smooth((age-.08)/.50)
  const release=smooth((age-DEALER_CARD_SNAP+.10)/.10)
  const grip=smooth((age-DEALER_CARD_SNAP+.10)/.10)
  const thumbGrip=smooth((age-DEALER_CARD_SNAP+.20)/.20)
  ...
  const flick=smooth((age-DEALER_CARD_SNAP+.18)/.18)*(1-smooth((age-DEALER_CARD_SNAP+.02)/.40))
  const after=Math.max(0,age-DEALER_CARD_REVEAL_START)
  const width=.25+.75*smooth(after/DEALER_CARD_UNFOLD)
  const fan=smooth((after-.02)/.20)+.06*Math.sin(Math.PI*smooth((after-.02)/.30))
