...
export const DEALER_CARD_SNAP=.55
...
export function dealerCardReveal(age=Infinity) {
  ...
  const prepare=smooth((age-.04)/.26)
  ...
  const rest=1-smooth(age/.30)
  // Overlap the end of the gather with the stroke: no loaded-pose hold.
  const wind=smooth((age-.04)/.33)*(1-smooth((age-DEALER_CARD_SNAP+.18)/.18))
