...
export function titleDepthTransform(camera:Vector3) {
  const scale=1.8
  return {scale,position:camera.clone().multiplyScalar(1-scale)}
}

export default function TitleDepth({children}:{children:ReactNode}) {
  ...
  useFrame(({camera})=>{
    ...
    const transform=titleDepthTransform(eye)
