...
assert.equal(dealerCardReveal(DEALER_CARD_SNAP).visible,true,'Both cards appear on the exact snap beat')
assert.equal(dealerCardReveal(DEALER_CARD_SNAP).width,1,'The snap reveals complete cards, without growing them')
assert.equal(dealerCardReveal(DEALER_CARD_SNAP).fan,0,'Cards appear stacked')
assert.ok(dealerCardReveal(DEALER_CARD_SNAP+.001).fan>0,'The spread starts immediately on appearance')
