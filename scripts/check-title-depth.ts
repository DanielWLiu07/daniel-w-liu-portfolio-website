...
for(const aspect of [1500/900,390/844]) for(const eye of [[0,4.6,12.5],[3,5,10],[-2,3,8]]) for(const depthScale of [.8,1.8]) {
  ...
  const {scale,position}=titleDepthTransform(camera.position,depthScale)
  for(const point of [[-4,3,0],[4,3,0],[0,4.5,0],[0,2.5,0],[-6,-3,2],[6,8,-2]]) {
    ...
    assert.ok(depthScale>1 ? after.z>before.z : after.z<before.z,'Depth ordering follows the responsive title layer')
  }
}
