...
import {Texture,SRGBColorSpace,type Mesh,type MeshStandardMaterial} from 'three'
...
const rig=new DealerShuffleRig(root),maps=[...DEALER_CARD_ART.map(()=>new Texture()),new Texture()]
...
mats.forEach((materials,i)=>{
 ...
 assert.equal(materials[1].map,maps[2],'The back uses the supplied shared flying-deck canvas texture')
})
...
let disposed=false;maps[2].addEventListener('dispose',()=>{disposed=true})
