...
// The scene now points to the red chip; keep this as a standalone pinch-rig study.
const applyChip=(seconds:number,amount=1,speed=1,acting=1,interactionBlend=1)=>{
 body.apply(seconds,amount,speed,acting,interactionBlend)
 body.chip.apply(seconds*speed,amount>0?interactionBlend:0)
}
...
for(let i=0;i<=720;i++) {
 const t=i/24;body.reset();applyChip(t)
...
applyChip(0,0);assert.equal(body.chip.group.visible,false)
body.reset();applyChip(0,.01,1,1,.2);assert.equal(body.chip.group.visible,false)
